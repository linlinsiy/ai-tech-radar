...
- **端口**：9003
- **鉴权**：无
- **功能**：读取正式本地快照并执行候选池、L2、L3 和受控导入；不重新采集。传入 `COL-*` 时执行首次 L2/L3 分析，并忽略历史已处理缓存、仅做批内去重，以便同一月度采集快照可先做周度验证再做月度分析；传入完整任务或先前分析生成的 `IMP-*`/`RERUN-*` 时按正式重分析语义重跑 L2/L3。成功后写入 internal/MySQL，并生成或更新正式分析快照。
