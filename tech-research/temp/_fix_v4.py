...
# Find articles block start
idx_start = c.find(b'"articles"')
if idx_start < 0:
    print("FAILED: articles not found")
    exit(1)

# Find the end of the JSON code block before the success response
# Pattern: }\n```\n\n- **成功响应**
end_marker = b'}\r\n```\r\n\r\n- **'
end_pos = c.find(end_marker, idx_start)
if end_pos < 0:
    end_marker = b'}\n```\n\n- **'
    end_pos = c.find(end_marker, idx_start)
if end_pos < 0:
    print("FAILED: end marker not found")
    print("Around articles end:", repr(c[idx_start+4000:idx_start+4200]))
    exit(1)

print(f"Replace from {idx_start} to {end_pos} (len={end_pos-idx_start})")
...
# Build NEW complete payload matching actual API schemas
new_payload = f'''"articles": [
    {{
      "url": "{url_xz}",
...
# Replace
c = c[:idx_start] + new_payload.encode('utf-8') + c[end_pos:]
f.write_bytes(c)
print("SUCCESS: Document updated with schema-compliant payload")
